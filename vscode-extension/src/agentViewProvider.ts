...
function getConfiguredBackendUrl(): string {
  ...
  return configuration.get<string>('backendUrl')?.trim() || DEFAULT_BACKEND_URL;
}

function getFallbackBackendUrl(): string {
  const configuration = vscode.workspace.getConfiguration('ronin');
  return configuration.get<string>('fallbackBackendUrl')?.trim() || FALLBACK_BACKEND_URL;
}

function getBackendUrls(path: string): string[] {
  ...
  const urls = [primary, secondary].filter(Boolean) as string[];
  return Array.from(new Set(urls));
}

export class RoninAgentProvider implements vscode.TreeDataProvider<vscode.TreeItem> {
  private readonly _onDidChangeTreeData = new vscode.EventEmitter<vscode.TreeItem | null>();
  ...
  createCollapsibleItem(label: string): vscode.TreeItem {
  ...
  async getProjectNodes(): Promise<vscode.TreeItem[]> {
  ...
  async applyWorkspaceEdit(filePath: string | undefined, content: string): Promise<string> {
    if (!content?.trim()) {
  ...
  async openProject(project: any): Promise<void> {
    ...
    const panel = vscode.window.createWebviewPanel(
      'roninProjectDetails',
      `Ronin Project: ${project.name || project.id}`,
  ...
  getProjectHtml(project: any, messages: any[] = []): string {
...
<body>
  ...
  async openAgentWebview(): Promise<void> {
    const panel = vscode.window.createWebviewPanel(
      'roninDashboard',
      'Ronin Dashboard',
      vscode.ViewColumn.One,
  ...
  getDashboardHtml(status: string, backendUrl: string): string {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
...
<body>
  ...
  async request(path: string, init?: RequestInit, useAuth: boolean = true): Promise<any> {
